 ...
 * @since 1.0.0
 */
class DataLevel3Avito extends DataLevel3Base
{
    ...
    protected static function getFloor(Document $doc) : string
    {
        ...
        foreach ($params as $param) {
            ...
            if (strpos($text, 'Этаж') !== false) {
                ...
                if (!empty($res)) {
                    return preg_replace('~[^0-9]~Uuis', '', $res[1]);
                }
            }
        }
    ...
    protected static function getNumberOfFloors(Document $doc) : string
    {
        ...
        foreach ($params as $param) {
            $text = $param->text();
            if (strpos($text, 'Этаж') !== false) {
                return preg_replace('~[^0-9]~Uuis', '', strstr($text, 'из'));
            }
        }
    ...
    {
        Yii::error($doc, 'getMetroStation1');
    ...
    {
        Yii::error($doc, 'getMetroStation2');
        $stations = $doc->find('.item-address-georeferences-item__content');
    ...
    {
        Yii::error($doc, 'getMetroStation3');
        $stations = $doc->find('.item-address-georeferences-item__content');
